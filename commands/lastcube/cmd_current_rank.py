...
async def handle(plugin, event: AstrMessageEvent):
    ...
    try:
        result = await plugin.lastcubex_client.get_current_ranking(normalized_event)
        if result.get("code") != 200:
            error_msg = result.get("message", "未知错误")
            yield event.plain_result(
                f"获取 LastCubeX 当前周赛榜失败\n错误：{error_msg}"
            ).use_t2i(False)
            return
        ...
        if not leaderboard:
            yield event.plain_result(
                f"Last cube {normalized_event} 当前周赛榜暂时还没有成绩"
            ).use_t2i(False)
            return

        lines = [f"Last cube {normalized_event} 当前周赛榜如下："]
        ...
        logger.error(f"LastCubeX 当前周赛榜命令异常: {exc}")
